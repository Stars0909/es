...
## (一)安装IK分词器
1.进入elasticsearch-8.12.2/bin目录，在目录栏中输入cmd进入执行下面命令行界面，
然后执行该命令：elasticsearch-plugin install https://get.infini.cloud/elasticsearch/analysis-ik/8.12.2
![img_42.png](pictures/img_42.png)  
![img_43.png](pictures/img_43.png)

2.在elasticsearch-8.12.2/config中可以看到analysis-ik目录就安装好了
...
```
![创建用户信息索引.png](pictures/img_1.png)
...
```
![创建订单目录索引.png](pictures/img_2.png)


修改索引——为user_information索引添加age字段
```
...
```
![修改索引——为user_information索引添加age字段.png](pictures/img_3.png)
...
GET /_cat/indices?v  
```
![查看所有索引.png](pictures/img_4.png)

删除order_records索引
```
DELETE /order_records
```
![删除order_records索引.png](pictures/img_5.png)
...
### 3.删除订单 OR002
### 4.查看订单 OR003 的详细信息
·第一条代码为删除订单 OR002，此处将不执行，以免影响后续操作  
·查看订单为第二条代码
![img_11.png](pictures/img_11.png)


## (四）高级查询与DSL练习
注：由于kibana工具在查询数据时默认最大查询10个结果，所以下面结果超过10个的都扩展到最大查询数据个数20(每个文档最多20个数据)。

### 1.用户信息数据
#### 1.1.查询所有女性用户的姓名和电子邮件。
```
...
#### 1.2. 查找最后登录日期在2024年9月1日之后的所有活跃用户。
```
POST /user_information/_search
{
  "query": {
...
#### 1.6. 查找注册日期在2023年1月1日到2023年12月31日之间的用户。
```
POST /user_information/_search
{
  "query": {
...
#### 1.9. 查询电子邮件域为"example.com"的所有用户。
```
POST /user_information/_search
{
  "query": {
...
### 3.订单记录数据
#### 3.1. 查询所有状态为"completed"的订单的订单ID和总金额。
```
...
#### 3.6. 查找所有状态为"cancelled"的订单的客户ID。
```
GET /order_records/_search
{
  "query": {
